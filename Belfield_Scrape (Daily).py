...
def send_email_notification(success, items_count):
    # Email details
    ...
    if success:
        msg['Subject'] = f" Web Scraper (Belfield Daily) Success: {items_count} items scraped"
        body = f"The web scraper ran successfully and processed {items_count} items."
    else:
        msg['Subject'] = "Web Scraper Failed"
        body = f"The web scraper (Belfield Daily) encountered an error: {e}. Please check the logs."
...
send_email_notification(True, items_scrapped)
    ...
    print(f"Error: {str(e)}")
    send_email_notification(False, 0)
